...
- **`docs`** — Next.js App Router app on port 3000. Serves as the documentation and live preview site for `@bambi-ui/*` packages.
  - Production URL: `https://bambi-ui.felekoglu.dev`
  - Component pages live under `app/(narrow)/components/<name>/page.tsx`. The `(narrow)` route group adds `max-w-3xl`; the theme page at `app/theme/page.tsx` is full-width.
  - The Theme Builder lives at `app/theme/page.tsx` and uses `components/ThemeBuilder.tsx`.
  - Docs-only React components (not exported from packages) live in `components/`.
  - `components/CodeBlock.tsx` — async server component using `shiki` for syntax highlighting.
  - `components/ReadmeBlock.tsx` — async server component using `unified`/`remark`/`rehype` + `@shikijs/rehype` to render package README.md files.
  - `components/SideNav.tsx` — `"use client"` component using `usePathname` for active-link highlighting.
  - Dark mode is handled via an inline `<script>` in `app/layout.tsx` (prevents FOUC) and the `ThemeToggle` client component.
...
- In `apps/docs`, component pages live at `app/(narrow)/components/<name>/page.tsx`. The `(narrow)` route group does not affect the URL.
- All `"use client"` directives belong in component packages as needed. The built dist files for packages that inject CSS include `"use client"` automatically (tsup includes it), so they work as client components in Next.js.
...
`apps/docs/app/layout.tsx` is the root layout. Narrow pages (most component pages) live inside the `(narrow)` route group which adds a `max-w-3xl` wrapper via `app/(narrow)/layout.tsx`. Wide pages (e.g. Theme Builder at `/theme`) live outside the `(narrow)` group and have no width constraint.
